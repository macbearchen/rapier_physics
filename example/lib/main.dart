...
void main() async {
  ...
  final cylinder = world.addCylinder(
    halfHeight: 1.0,
    radius: 0.5,
    desc: RigidBodyDesc.dynamic()..position = Vector3(0, 20, 0),
  );

  // Create a dynamic capsule at y = 25.0
  final capsule = world.addCapsule(
    halfHeight: 1.0,
    radius: 0.5,
    desc: RigidBodyDesc.dynamic()..position = Vector3(0, 25, 0),
  );

  debugPrint('Initial box Y: ${box.position.y}');
  debugPrint('Initial sphere Y: ${sphere.position.y}');
  debugPrint('Initial cylinder Y: ${cylinder.position.y}');
  debugPrint('Initial capsule Y: ${capsule.position.y}');
  ...
  debugPrint('Final box Y: ${box.position.y}');
  debugPrint('Final sphere Y: ${sphere.position.y}');
  debugPrint('Final cylinder Y: ${cylinder.position.y}');
  debugPrint('Final capsule Y: ${capsule.position.y}');
